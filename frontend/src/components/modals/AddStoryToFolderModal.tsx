...
const StorySelectionItem = styled.div<{ $selected: boolean }>`
  ...
  border-radius: 8px;
  background: ${(props) =>
    props.$selected ? "rgba(76, 175, 80, 0.2)" : "rgba(255, 255, 255, 0.03)"};
  border: 1px solid
    ${(props) => (props.$selected ? "#4CAF50" : "rgba(255, 255, 255, 0.1)")};
  cursor: pointer;
  margin-bottom: 8px;
  ...
  &:hover {
    background: ${(props) =>
      props.$selected ? "rgba(76, 175, 80, 0.3)" : "rgba(255, 255, 255, 0.06)"};
  }
`;
  ...
  return (
    <Modal onClick={onClose}>
      <ModalContent onClick={(e) => e.stopPropagation()}>
        ...
        <div
          style={{ maxHeight: "400px", overflowY: "auto", marginBottom: 20 }}
        >
          {stories.map((story) => (
            <StorySelectionItem
              key={story.id}
